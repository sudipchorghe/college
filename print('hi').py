...
print("Select type of resistor: enter '1' for 4-band and '2' for 5-band   :")
choice = int(input())
...
if choice == 1:
    ...
    def res_val2(band1, band2, band3, band4, band5):
        val = (colorband[band1]*100 + colorband[band2]*10 + colorband[band3]) * multiplier[band4]
        tol = tolerance.get(band5)
        print("resistance calc")
        return val, tol

    b1 = input("Band 1 color: ").lower()
    b2 = input("Band 2 color: ").lower()
    b3 = input("Band 3 color: ").lower()
    b4 = input("Multiplier band color: ").lower()
    b5 = input("Tolerance band color: ").lower()

    try:
        value, tol = res_val2(b1, b2, b3, b4, b5)
        print(f"The resistor value is {value} Ohms with a tolerance of {tol}.")
    except KeyError:
        print("---------------enter a valid color broski--------------------")